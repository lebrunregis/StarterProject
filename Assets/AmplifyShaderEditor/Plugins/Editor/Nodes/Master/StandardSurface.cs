...
namespace AmplifyShaderEditor
{
    ...
    public sealed class StandardSurfaceOutputNode : MasterNode, ISerializationCallbackReceiver
    {
        ...
        private readonly static GUIContent RenderPathContent = new("Render Path", "Selects and generates passes for the supported rendering paths\nDefault: All");
        private const string ShaderModelStr = "Shader Model";
        private readonly static GUIContent LightModelContent = new("Light Model", "Surface shader lighting model defines how the surface reflects light\nDefault: Standard");
        private readonly static GUIContent ShaderLODContent = new("Shader LOD", "Shader LOD");
        private readonly static GUIContent CullModeContent = new("Cull Mode", "Polygon culling mode prevents rendering of either back-facing or front-facing polygons to save performance, turn it off if you want to render both sides\nDefault: Back");
        ...
        private readonly static GUIContent AlphaModeContent = new(" Blend Mode", "Defines how the surface blends with the background\nDefault: Opaque");
        private const string OpacityMaskClipValueStr = "Mask Clip Value";
        private readonly static GUIContent OpacityMaskClipValueContent = new("Mask Clip Value", "Default clip value to be compared with opacity alpha ( 0 = fully Opaque, 1 = fully Masked )\nDefault: 0.5");
        private readonly static GUIContent CastShadowsContent = new("Cast Shadows", "Generates a shadow caster pass for vertex modifications and point lights in forward rendering\nDefault: ON");
        private readonly static GUIContent ReceiveShadowsContent = new("Receive Shadows", "Untick it to disable shadow receiving, this includes self-shadowing (only for forward rendering) \nDefault: ON");
        private readonly static GUIContent QueueIndexContent = new("Queue Index", "Value to offset the render queue, accepts both positive values to render later and negative values to render sooner\nDefault: 0");
        private readonly static GUIContent RefractionLayerStr = new("Refraction Layer", "Use it to group or ungroup different refraction shaders into the same or different grabpass (only for forward rendering) \nDefault: 0");
        private readonly static GUIContent AlphaToCoverageStr = new("Alpha To Coverage", "");
        private readonly static GUIContent RenderQueueContent = new("Render Queue", "Base rendering queue index\n(Background = 1000, Geometry = 2000, AlphaTest = 2450, Transparent = 3000, Overlay = 4000)\nDefault: Geometry");
        private readonly static GUIContent RenderTypeContent = new("Render Type", "Categorizes shaders into several predefined groups, usually to be used with screen shader effects\nDefault: Opaque");

        private const string ShaderInputOrderStr = "Shader Input Order";


        [SerializeField]
        private BlendOpsHelper m_blendOpsHelper = new();

        [SerializeField]
        private StencilBufferOpHelper m_stencilBufferHelper = new();

        [SerializeField]
        private ZBufferOpHelper m_zBufferHelper = new();

        [SerializeField]
        private OutlineOpHelper m_outlineHelper = new();

        [SerializeField]
        private TessellationOpHelper m_tessOpHelper = new();

        [SerializeField]
        private ColorMaskHelper m_colorMaskHelper = new();

        [SerializeField]
        private RenderingPlatformOpHelper m_renderingPlatformOpHelper = new();

        [SerializeField]
        private RenderingOptionsOpHelper m_renderingOptionsOpHelper = new();

        [SerializeField]
        private BillboardOpHelper m_billboardOpHelper = new();

        [SerializeField]
        private FallbackPickerHelper m_fallbackHelper = null;

        [SerializeField]
        private TerrainDrawInstancedHelper m_drawInstancedHelper = new();

        //legacy
        [SerializeField]
        private AdditionalIncludesHelper m_additionalIncludes = new();
        //legacy
        [SerializeField]
        private AdditionalPragmasHelper m_additionalPragmas = new();
        //legacy
        [SerializeField]
        private AdditionalDefinesHelper m_additionalDefines = new();

        [SerializeField]
        private TemplateAdditionalDirectivesHelper m_additionalDirectives = new(" Additional Directives");

        [SerializeField]
        private AdditionalSurfaceOptionsHelper m_additionalSurfaceOptions = new();

        [SerializeField]
        private UsePassHelper m_usePass;

        [SerializeField]
        private CustomTagsHelper m_customTagsHelper = new();

        [SerializeField]
        private DependenciesHelper m_dependenciesHelper = new();
        ...
        [SerializeField]
        private InlineProperty m_inlineCullMode = new();

        [SerializeField]
        private InlineProperty m_inlineChromaticAberration = new(0.1f);
        ...
        [SerializeField]
        private InlineProperty m_inlineOpacityMaskClipValue = new();

        [SerializeField]
        private InlineProperty m_inlineAlphaToCoverage = new();
        ...
        [SerializeField]
        private CacheNodeConnections m_cacheNodeConnections = new();
        ...
        public override void OnNodeLogicUpdate(DrawInfo drawInfo)
        {
            ...
            if (m_checkChanges)
            {
                if (m_translucencyPort.IsConnected)
                {
                    ...
                    {
                        List<PropertyNode> translucencyList = new();
                        for (int i = 0; i < 7; i++)
                        {
                ...
                if (m_refractionPort.IsConnected)
                {
                    ...
                    {
                        List<PropertyNode> refractionList = new();
                        for (int i = 0; i < 2; i++)
                        {
                ...
                if (m_tessOpHelper.EnableTesselation && !m_tessellationPort.IsConnected)
                {
                    ...
                    {
                        List<PropertyNode> tessellationList = new();
                        for (int i = 0; i < 4; i++)
                        {
        ...
        public override Shader Execute(string pathname, bool isFullPath)
        {
            ...
            SortedList<int, InputPort> sortedPorts = new();
            for (int i = 0; i < m_inputPorts.Count; i++)
            {
