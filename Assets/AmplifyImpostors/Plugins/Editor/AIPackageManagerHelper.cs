...
namespace AmplifyImpostors
{
    ...
    {
        private static readonly string URPPackageId = "com.unity.render-pipelines.universal";
        private static readonly string HDRPPackageId = "com.unity.render-pipelines.high-definition";

        private static readonly string NewVersionDetectedFormat = "[AmplifyImpostors] A new {0} version {1} was detected and compatible shaders are being imported.\nPlease hit the Update button on your ASE canvas to recompile your shader under the newest version.";
        ...
        private static readonly Dictionary<int, AISRPPackageDesc> m_srpPackageSupport = new()
        {
        ...
        private static void WaitForPackageListBeforeUpdating()
        {
        ...
        private static void FailedPackageImport(string packageName, string errorMessage)
        {
            FinishImporter();
        }

        private static void CancelledPackageImport(string packageName)
        {
            FinishImporter();
        }

        private static void CompletedPackageImport(string packageName)
        {
            FinishImporter();
        }
        ...
        private static void ApplySRP()
        {
            ...
            if (cgincMatch.Success)
            {
                ...
                if (cgincSRPversion != m_packageHDRPVersion.ToString())
                {
                    cginc = cginc.Replace(cgincMatch.Groups[0].Value, "#define AI_HDRP_VERSION " + m_packageHDRPVersion.ToString());
                    saveAndRefresh = true;
                }
            }
            ...
            {
                string cgincSRPversion = cgincMatch.Groups[1].Value;
                if (cgincSRPversion != m_packageURPVersion.ToString())
                {
                    cginc = cginc.Replace(cgincMatch.Groups[0].Value, "#define AI_URP_VERSION " + m_packageURPVersion.ToString());
                    saveAndRefresh = true;
                }
            }
