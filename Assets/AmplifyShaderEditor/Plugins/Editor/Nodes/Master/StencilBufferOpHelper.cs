...
namespace AmplifyShaderEditor
{
    ...
    public class StencilBufferOpHelper
    {
        ...
        public static readonly Dictionary<string, int> StencilComparisonValuesDict = new()
        {
            {"Greater" , 1},
            {"GEqual" ,  2},
            {"Less" ,    3},
        ...
        public static readonly Dictionary<string, int> StencilOpsValuesDict = new()
        {
            {"Keep",    1},
            {"Zero",    2},
            {"Replace", 3},
        ...
        private readonly GUIContent ReferenceValueContent = new("Reference", "The value to be compared against (if Comparison is anything else than always) and/or the value to be written to the buffer (if either Pass, Fail or ZFail is set to replace)");
        private readonly GUIContent ReadMaskContent = new("Read Mask", "An 8 bit mask as an 0-255 integer, used when comparing the reference value with the contents of the buffer (referenceValue & readMask) comparisonFunction (stencilBufferValue & readMask)");
        private readonly GUIContent WriteMaskContent = new("Write Mask", "An 8 bit mask as an 0-255 integer, used when writing to the buffer");
        ...
        [SerializeField]
        private InlineProperty m_refValue = new(ReferenceDefaultValue);
        [SerializeField]
        private InlineProperty m_readMask = new(ReadMaskDefaultValue);
        [SerializeField]
        private InlineProperty m_writeMask = new(WriteMaskDefaultValue);

        //Comparison Function
        [SerializeField]
        private InlineProperty m_comparisonFunctionIdx = new(ComparisonDefaultValue);
        [SerializeField]
        private InlineProperty m_comparisonFunctionBackIdx = new(ComparisonDefaultValue);

        //Pass Stencil Op
        [SerializeField]
        private InlineProperty m_passStencilOpIdx = new(PassStencilOpDefaultValue);
        [SerializeField]
        private InlineProperty m_passStencilOpBackIdx = new(PassStencilOpDefaultValue);

        //Fail Stencil Op 
        [SerializeField]
        private InlineProperty m_failStencilOpIdx = new(FailStencilOpDefaultValue);
        [SerializeField]
        private InlineProperty m_failStencilOpBackIdx = new(FailStencilOpDefaultValue);

        //ZFail Stencil Op
        [SerializeField]
        private InlineProperty m_zFailStencilOpIdx = new(ZFailStencilOpDefaultValue);
        [SerializeField]
        private InlineProperty m_zFailStencilOpBackIdx = new(ZFailStencilOpDefaultValue);
