...
namespace AmplifyShaderEditor
{
    ...
    public class Clipboard
    {
        ...
        private readonly List<ClipboardData> m_clipboardStrData;
        private readonly Dictionary<int, ClipboardData> m_clipboardAuxData;
        private readonly Dictionary<string, ClipboardData> m_multiPassMasterNodeData;
        ...
        public void AddMultiPassNodesToClipboard(List<TemplateMultiPassMasterNode> masterNodes, bool resetList, int lodId)
        {
            ...
            for (int i = 0; i < templatesAmount; i++)
            {
                if (!masterNodes[i].InvalidNode)
                {
                    ...
                    ClipboardData clipboardData = new(data, connection, masterNodes[i].UniqueId);
                ...
                for (int i = 1; i < clipboardDataArray.Length; i++)
                {
                    if (!string.IsNullOrEmpty(clipboardDataArray[i]))
                    {
                        ...
                        if (nodeDataArr.Length > 2)
                        {
                            ...
                            ClipboardData data = new(nodeData, connections, nodeId);
                            m_clipboardStrData.Add(data);
