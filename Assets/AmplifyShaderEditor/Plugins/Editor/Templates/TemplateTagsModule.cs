...
namespace AmplifyShaderEditor
{
    ...
    public class TemplateTagsModule : TemplateModuleParent
    {
        ...
        [SerializeField]
        private List<CustomTagData> m_availableTags = new();

        private Dictionary<string, CustomTagData> m_availableTagsDict = new();
        ...
        public void CopyFrom(TemplateTagsModule other)
        {
            ...
            {
                CustomTagData newData = new(other.AvailableTags[i]);
        ...
        public void ConfigureFromTemplateData(TemplateTagsModuleData tagsData)
        {
            ...
            if (newValidData && newValidData != m_validData)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    CustomTagData tagData = new(tagsData.Tags[i].Name, tagsData.Tags[i].Value, i);
        ...
        private void AddTagFromRead(string data)
        {
            ...
            if (arr.Length > 1)
            {
                ...
                {
                    CustomTagData tagData = new(data, m_availableTags.Count);
                    m_availableTags.Add(tagData);
