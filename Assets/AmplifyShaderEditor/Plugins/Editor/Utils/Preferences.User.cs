...
namespace AmplifyShaderEditor
{
    public partial class Preferences
    {
        ...
        public class User
        {
            ...
            {
                public static readonly GUIContent StartUp = new("Show start screen on Unity launch", "You can set if you want to see the start screen everytime Unity launchs, only just when there's a new version available or never.");
                public static readonly GUIContent AlwaysSnapToGrid = new("Always Snap to Grid", "Always snap to grid when dragging nodes around, instead of using control.");
                public static readonly GUIContent EnableUndo = new("Enable Undo (unstable)", "Enables undo for actions within the shader graph canvas. Currently unstable, use with caution.");
                public static readonly GUIContent ClearLog = new("Clear Log on Update", "Clears the previously generated log each time the Update button is pressed.");
                public static readonly GUIContent LogShaderCompile = new("Log Shader Compile", "Log message to console when a shader compilation is finished.");
                public static readonly GUIContent LogBatchCompile = new("Log Batch Compile", "Log message to console when a batch compilation is finished.");
                public static readonly GUIContent UpdateOnSceneSave = new("Update on Scene save (Ctrl+S)", "ASE is aware of Ctrl+S and will use it to save shader.");
                public static readonly GUIContent PreviewUpdateFrequency = new("Preview Update Frequency", "Frequency limit, in Hz/FPS, at which we allow previews to refresh.");
                public static readonly GUIContent DisablePreviews = new("Disable Node Previews", "Disable preview on nodes from being updated to boost up performance on large graphs.");
                public static readonly GUIContent DisableMaterialMode = new("Disable Material Mode", "Disable enter Material Mode graph when double-clicking on material asset.");
                public static readonly GUIContent ForceTemplateMinShaderModel = new("Force Template Min. Shader Model", "If active, when loading a shader its shader model will be replaced by the one specified in template if what is loaded is below the one set over the template.");
                public static readonly GUIContent ForceTemplateInlineProperties = new("Force Template Inline Properties", "If active, defaults all inline properties to template values.");
            }
            ...
            private static readonly string[] FrequencyOptions = { "30 hz", "60 hz", "120 hz", "240 hz", "Unlimited" };
            private static readonly int[] FrequencyOptionsIndexToValue = { 30, 60, 120, 240, 10000 };
            private static readonly Dictionary<int, int> FrequencyOptionsValueToIndex = new()
            {
