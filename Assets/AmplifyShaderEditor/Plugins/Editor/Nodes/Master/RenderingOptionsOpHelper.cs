...
namespace AmplifyShaderEditor
{
    ...
    public class RenderingOptionsOpHelper
    {
        ...
        private readonly static GUIContent EmissionGIFlags = new("Emission GI Flag", "Modifies Emission GI flags");
        private readonly static GUIContent LODCrossfadeContent = new(" LOD Group Cross Fade", "Applies a dither crossfade to be used with LOD groups for smoother transitions. Uses one interpolator\nDefault: OFF");
        private readonly static GUIContent DisableBatchingContent = new("Disable Batching", "\nDisables objects to be batched and used with DrawCallBatching Default: False");
        private readonly static GUIContent IgnoreProjectorContent = new(" Ignore Projector", "\nIf True then an object that uses this shader will not be affected by Projectors Default: False");
        private readonly static GUIContent UseDefaultCasterContent = new(" Use Default Shadow Caster", "\nIf True always use surface default shadow caster Default: False");
        private readonly static GUIContent ForceNoShadowCastingContent = new(" Force No Shadow Casting", "\nIf True then an object that is rendered using this subshader will never cast shadows Default: False");
        private readonly static GUIContent ForceEnableInstancingContent = new(" Force Enable Instancing", "\nIf True forces instancing on shader independent of having instanced properties");
        private readonly static GUIContent ForceDisableInstancingContent = new(" Force Disable Instancing", "\nIf True forces disable instancing on shader independent of having instanced properties");
        private readonly static GUIContent SpecularHightlightsContent = new(" Fwd Specular Highlights Toggle", "\nIf True creates a material toggle to set Unity's internal specular highlight rendering keyword");
        private readonly static GUIContent ReflectionsContent = new(" Fwd Reflections Toggle", "\nIf True creates a material toggle to set Unity's internal reflections rendering keyword");
