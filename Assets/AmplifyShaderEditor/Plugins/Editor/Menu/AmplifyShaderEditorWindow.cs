...
namespace AmplifyShaderEditor
{
    ...
    public class AmplifyShaderEditorWindow : SearchableEditorWindow, ISerializationCallbackReceiver
    {
        ...
        [SerializeField]
        private TemplatesManager m_templatesManager;
        ...
        private GenericMessageUI m_genericMessageUI;
        ...
        private readonly AutoPanData[] m_autoPanArea;

        private readonly DrawInfo m_drawInfo;
        private KeyCode m_lastKeyPressed = KeyCode.None;
        private readonly System.Type m_commentaryTypeNode;
        ...
        private List<Material> m_materialsToUpdate = new();
        ...
        private readonly GUIContent NodesExceptionMessage = new("ASE is unable to load correctly due to some faulty other classes/plugin in your project. We advise to review all your imported plugins.");
        ...
        private readonly NodeWireReferencesUtils m_wireReferenceUtils = new();
        ...
        [NonSerialized]
        public Dictionary<string, bool> VisitedChanged = new();

        [SerializeField]
        private List<Toast> m_messages = new();
        ...
        [NonSerialized]
        private readonly Dictionary<string, OutputPort> m_savedList = new();
        ...
        private static readonly System.Diagnostics.Stopwatch m_batchTimer = new();

        // Unity Menu item
        [MenuItem("Window/Amplify Shader Editor/Open Canvas", false, 1000)]
        private static void OpenMainShaderGraph()
        {
        ...
        {
            GUIContent content = new(original);
        ...
        private static void CreateConfirmationStandardShader()
        {
        ...
        private static void CreateNewShader(string customPath, string customShaderName)
        {
        ...
        private static void CreateNewShaderFunction()
        {
        ...
        private ShaderLoadResult OnSaveShader(bool value, Shader shader, Material material, AmplifyShaderFunction function)
        {
            if (value)
            {
                SaveToDisk(false);
                ...
                case ToolButtonType.TakeScreenshot:
                    {
                        ...
                        WindowsUtil.SetWindowLong(m_aseHandle, WindowsUtil.GWL_STYLE, windowLong & ~(WindowsUtil.WS_SIZEBOX));
                        ...
                        WindowsUtil.SetWindowLong(m_aseHandle, WindowsUtil.GWL_STYLE, windowLong);
...
#if UNITY_EDITOR_WIN
        private IntPtr FindASEWindowHandle()
        {
        ...
        private void TakeScreenShot()
        {
            ...
            Texture2D m_screenshotTex2D = new((int)position.width, (int)position.height, TextureFormat.RGB24, false);
        ...
        private void FocusZoom(bool forceAllNodes, bool doubleTap, bool smooth = true, float maxZoom = float.MaxValue)
        {
        ...
        private void SmoothZoom(float newZoom)
        {
            m_smoothZoom = true;
            m_zoomTime = 0;
        ...
        private void SmoothCameraOffset(Vector2 newOffset)
        {
            m_smoothOffset = true;
            m_offsetTime = 0;
        ...
        private void PreTestLeftMouseDown()
        {
                ...
                else
                {
                    ...
                    if (wireRefs != null && wireRefs.Count > 0)
                    {
                        for (int i = 0; i < wireRefs.Count; i++)
                        {
                            ...
                            Vector3 endPos = new(inputPort.Position.x, inputPort.Position.y);
                            Vector3 startPos = new(outputPort.Position.x, outputPort.Position.y);
                            ...
                            Vector3 startTangent = new(startPos.x + resizedMag, startPos.y);
                            Vector3 endTangent = new(endPos.x - resizedMag, endPos.y);
        ...
        private bool InsideMenus(Vector2 position)
        {
        ...
        private void DeleteSelectedNodes()
        {
            ...
            List<ParentNode> selectedNodeList = new(m_mainGraphInstance.SelectedNodes.Count);
            ...
            List<ParentNode> extraNodes = new();
        ...
        private bool OnKeyboardPress(KeyCode code)
        {
            return (m_currentEvent.keyCode == code && m_lastKeyPressed == KeyCode.None);
        }

        private void CheckKeyboardCameraDown()
        {
        ...
        private void CheckKeyboardCameraUp()
        {
            switch (m_currentEvent.keyCode)
            {
        ...
        private IEnumerator SendPostCoroutine(string url)
        {
            WWWForm form = new();
            ...
            if (!m_coroutine.MoveNext() && www != null)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            System.Text.RegularExpressions.Regex parser = new(@".*(http:\/\/paste.amplify.pt\/view\/)([0-9a-z]*).*", System.Text.RegularExpressions.RegexOptions.Singleline);
        ...
        private void ModifyZoom(float zoomIncrement, Vector2 pivot)
        {
        ...
        private void ModifyZoomSmooth(float zoomIncrement, Vector2 pivot)
        {
        ...
        private ParentNode CreateNodeFromClipboardData(int clipId)
        {
        ...
        private void CreateConnectionsFromClipboardData(int clipId)
        {
        ...
        private IEnumerator SendGetCoroutine(string url)
        {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (data.IndexOf(Clipboard.ClipboardId + ";") > -1)
                        {
                            ...
                            {
                                System.Text.RegularExpressions.Regex parser = new(@"(.*)<div .*", System.Text.RegularExpressions.RegexOptions.Singleline);
        ...
        private void ReconnectClipboardReferences(List<ParentNode> createdNodes)
        {
        ...
        private void PasteFromClipboard(bool copyConnections)
        {
            ...
            List<ParentNode> createdNodes = new();
        ...
        private void OnGUI()
        {
            ...
            if (m_forcingMaterialUpdateFlag)
            {
                ...
                {
                    float percentage = 100.0f * (UIUtils.TotalExampleMaterials - m_materialsToUpdate.Count) / UIUtils.TotalExampleMaterials;
        ...
        private readonly bool m_markToClose = false;
        private List<string> m_assetPaths = new();
        ...
        private void OnNodeStoppedMovingEvent(ParentNode node)
        {
            CheckZoomBoundaries(node.Vec2Position);
            //ShaderIsModified = true;
        }

        private void OnRefreshFunctionNodeEvent(FunctionNode node)
        {
            Debug.Log(node);
        }
        ...
        private void OnMaterialUpdated(MasterNode masterNode)
        {
            if (masterNode != null)
            {
        ...
        private void OnShaderUpdated(MasterNode masterNode)
        {
        ...
        private void OnEmptyGraphDetected(ParentGraph graph)
        {
        ...
        private void CheckNodeReplacement()
        {
