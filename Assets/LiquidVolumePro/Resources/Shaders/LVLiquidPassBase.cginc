...
#define LIQUID_VOLUME_IRREGULAR !(LIQUID_VOLUME_SPHERE || LIQUID_VOLUME_CUBE || LIQUID_VOLUME_CYLINDER)
 ...
 // Open GL 2.0 / WebGL support
	...
	#define SAMPLE_NOISE_3D(x,y) 0.5
		...
		half4 frag (Input IN) : SV_Target {
			...
			#ifdef USES_RIM
				half rim = pow(1.0 - saturate(dot(IN.normalWS, -rd)), _RimPower);
				co.rgb = lerp(co.rgb, _RimColor.rgb, _RimColor.a * rim);
			#endif			
