...
namespace AmplifyImpostors
{
    ...
    {
        private static readonly byte[] Footer = { 0x54, 0x52, 0x55, 0x45, 0x56, 0x49, 0x53, 0x49, 0x4F, 0x4E, 0x2D, 0x58, 0x46, 0x49, 0x4C, 0x45, 0x2E, 0x00 }; // TRUEVISION-XFILE.\0 signature (new TGA format)
        ...
        public static byte[] EncodeToTGA(this Texture2D tex, Compression compression = Compression.RLE)
        {
            ...
            using (MemoryStream stream = new(headerSize + tex.width * tex.height * bytesPerPixel))
            {
    ...
    public static class Vector2Ex
    {
        ...
        public static Vector2[] ConvexHull(Vector2[] P)
        {
            if (P.Length > 1)
            {
                ...
                Comparison<Vector2> comparison = new((a, b) =>
                {
                    if (a.x == b.x)
        ...
        public static Vector2[] ReduceVertices(Vector2[] P, int maxVertices)
        {
            ...
            {
                // turn into a box
                Rect newBox = new(P[0].x, P[0].y, 0f, 0f);
                for (int i = 0; i < P.Length; i++)
                {
                ...
                P = new Vector2[]
                {
                    new(newBox.xMin, newBox.yMin),
                    new(newBox.xMax, newBox.yMin),
                    new(newBox.xMax, newBox.yMax),
                    new(newBox.xMin, newBox.yMax),
                };
            }
            else
            {
