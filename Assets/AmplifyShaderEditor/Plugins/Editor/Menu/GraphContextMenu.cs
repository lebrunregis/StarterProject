...
namespace AmplifyShaderEditor
{
    ...
    public class GraphContextMenu
    {
        ...
        private readonly ParentGraph m_currentGraph;
        private readonly bool m_correctlyLoaded = false;
        ...
        public bool RefreshNodes(ParentGraph currentGraph)
        {
                    ...
                    if (!m_itemsDict.ContainsKey(type))
                    {
                        foreach (NodeAttributes attribute in Attribute.GetCustomAttributes(type).OfType<NodeAttributes>())
                        {
                            if (attribute.Available && !attribute.Deprecated)
                            {
                                ...
                                ContextMenuItem newItem = new(attribute, type, attribute.Name, attribute.Tags, attribute.Category, attribute.Description, null, attribute.ShortcutKey);
            ...
            List<AmplifyShaderFunction> allFunctions = new();
            ...
            for (int i = 0; i < functionCount; i++)
            {
                ...
                {
                    NodeAttributes attribute = new(allFunctions[i].FunctionName, allFunctions[i].CustomNodeCategory, allFunctions[i].Description, KeyCode.None, true, 0, int.MaxValue, typeof(AmplifyShaderFunction));
                    System.Type type = typeof(FunctionNode);

                    ContextMenuItem newItem = new(attribute, type, AddSpacesToSentence(attribute.Name), attribute.Tags, attribute.Category, attribute.Description, allFunctions[i], attribute.ShortcutKey);
                    m_items.Add(newItem);
                    m_itemFunctions.Add(newItem);
                }
            }
