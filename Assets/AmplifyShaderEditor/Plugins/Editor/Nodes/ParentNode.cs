...
namespace AmplifyShaderEditor
{
    ...
    public class ParentNode : UndoParentNode, ISerializationCallbackReceiver
    {
        ...
        protected GUIContent PrecisionContent = new("Precision", "Changes the precision of internal calculations, using lower types saves some performance\nDefault: Float");
        ...
        // Ports
        [SerializeField]
        protected List<InputPort> m_inputPorts = new();
        protected Dictionary<int, InputPort> m_inputPortsDict = new();

        [SerializeField]
        protected bool m_sortInputPorts = false;
        private List<InputPort> m_sortedInputPorts = new();

        [SerializeField]
        protected List<OutputPort> m_outputPorts = new();
        protected Dictionary<int, OutputPort> m_outputPortsDict = new();

        [SerializeField]
        protected bool m_sortOutputPorts = false;
        private List<OutputPort> m_sortedOutputPorts = new();
        ...
        private readonly GUIContent m_errorIcon = new();
        private readonly GUIContent m_errorMessage = new();
        ...
        private readonly bool[] m_previewChannels = { true, true, true, false };
        ...
        private void RecalculateInputPortIdx()
        {
            m_inputPortsDict.Clear();
        ...
        {
            InputPort port = new(m_uniqueId, (uniquePortId < 0 ? m_inputPorts.Count : uniquePortId), type, name, typeLocked, (orderId >= 0 ? orderId : m_inputPorts.Count), category);
        ...
        {
            InputPort port = new(m_uniqueId, (uniquePortId < 0 ? m_inputPorts.Count : uniquePortId), type, name, dataName, typeLocked, (orderId >= 0 ? orderId : m_inputPorts.Count), category);
            m_inputPorts.Add(port);
            m_inputPortsDict.Add(port.PortId, port);
            SetSaveIsDirty();
            m_sizeIsDirty = true;
        ...
        {
            OutputPort port = new(this, m_uniqueId, (uniquePortId < 0 ? m_outputPorts.Count : uniquePortId), type, name);
        ...
        public bool ShowTooltip(DrawInfo drawInfo)
        {
            ...
            if (m_globalPosition.Contains(drawInfo.MousePosition) || m_linkVisibility)
            {
                if (m_tooltipTimestamp + 0.6f < Time.realtimeSinceStartup || m_linkVisibility)
                {
                    ...
                    GUIContent temp = new(errorTooltip ? m_errorMessageTooltip : m_tooltipText);
        ...
        {
            OutputPort port = GetOutputPortByUniqueId(outputArrayId);
            if ((int)port.DataType >= 1 << 10 || port.DataType == WirePortDataType.OBJECT) //10 is the flag start of sampler types
            {
                port.SetLocalValue(value, dataCollector.PortCategory);
                return;
            }
        ...
        public Dictionary<string, InputPort> InputPortsDict
        {
            get
            {
                Dictionary<string, InputPort> dict = new();
                for (int i = 0; i < m_inputPorts.Count; i++)
                {
