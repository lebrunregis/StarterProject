...
namespace AmplifyShaderEditor
{
    ...
    public sealed class TriplanarNode : ParentNode
    {
        ...
        private bool m_texturesInitialize = false;
        ...
        private readonly List<string> m_functionSamplingBodyProj = new() {
            "float3 projNormal = ( pow( abs( worldNormal ), falloff ) );",
            "projNormal /= ( projNormal.x + projNormal.y + projNormal.z ) + 0.00001;",// 0.00001 is to prevent division by 0
			"float3 nsign = sign( worldNormal );"
        };

        private readonly List<string> m_functionSamplingBodyNegProj = new() {
            "float negProjNormalY = max( 0, projNormal.y * -nsign.y );",
            "projNormal.y = max( 0, projNormal.y * nsign.y );"
        };

        private readonly List<string> m_functionSamplingBodySignsSphere = new() {
        ...
        private readonly List<string> m_functionSamplingBodySignsSphereScale = new() {
        ...
        private readonly List<string> m_functionSamplingBodySignsCylinder = new() {
            "yNormN.xyz = half3( {0}( yNormN {1}).xy * float2( nsign.y, 1.0 ) + worldNormal.xz, worldNormal.y ).xzy;"
        };

        private readonly List<string> m_functionSamplingBodySignsCylinderScale = new() {
            "yNormN.xyz = half3( {0}( yNormN, normalScale.z ).xy * float2( nsign.y, 1.0 ) + worldNormal.xz, worldNormal.y ).xzy;"
        };

        private readonly List<string> m_functionSamplingBodyReturnSphereNormalize = new() {
            "return normalize( xNorm.xyz * projNormal.x + yNorm.xyz * projNormal.y + zNorm.xyz * projNormal.z );"
        };

        private readonly List<string> m_functionSamplingBodyReturnCylinderNormalize = new() {
            "return normalize( xNorm.xyz * projNormal.x + yNorm.xyz * projNormal.y + yNormN.xyz * negProjNormalY + zNorm.xyz * projNormal.z );"
        };

        private readonly List<string> m_functionSamplingBodyReturnSphere = new() {
            "return xNorm * projNormal.x + yNorm * projNormal.y + zNorm * projNormal.z;"
        };

        private readonly List<string> m_functionSamplingBodyReturnCylinder = new() {
        ...
        private void ConvertListTo(MasterNodeDataCollector dataCollector, bool scaleInfo, List<string> original, List<string> dest)
        {
        ...
        private void DrawTopTextureOptions()
        {
            EditorGUI.BeginChangeCheck();
        ...
        private void DrawMidTextureOptions()
        {
            if (m_midTexture == null)
                return;
        ...
        private void DrawBotTextureOptions()
        {
            if (m_botTexture == null)
                return;
        ...
        public override void Draw(DrawInfo drawInfo)
        {
            ...
            if (m_editing)
            {
                ...
                if (commandName.Equals("ObjectSelectorUpdated") || commandName.Equals("ObjectSelectorClosed"))
                {
                    ...
                    if (m_pickId == 2)
                    {
                        if (newValue != m_botTexture.Value)
                        {
                            PreviewIsDirty = true;
                    ...
                    {
                        if (newValue != m_midTexture.Value)
                        {
                            PreviewIsDirty = true;
                            UndoRecordObject("Changing value EditorGUIObjectField on node Triplanar Node");
                    ...
                    {
                        if (newValue != m_topTexture.Value)
                        {
                            PreviewIsDirty = true;
                            UndoRecordObject("Changing value EditorGUIObjectField on node Triplanar Node");
