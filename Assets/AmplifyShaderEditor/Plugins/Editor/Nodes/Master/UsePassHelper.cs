...
namespace AmplifyShaderEditor
{
    ...
    public class UsePassHelper : ScriptableObject
    {
        ...
        [SerializeField]
        private List<UsePassItem> m_items = new();
                ...
                m_reordableList = new ReorderableList(m_items, typeof(UsePassItem), true, false, false, false)
                {
                    ...
                    drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
                    {
                        if (m_items[index] != null)
                        {
                            ...
                            Rect popupPos = new(rect.x, rect.y + 2, popUpWidth * rect.width, rect.height);
                            Rect labelPos = new(rect.x + popupPos.width * labelWidthMultiplier, rect.y, 0.59f * rect.width, rect.height);

                            Rect shaderSelectorPos = new(labelPos.x + labelPos.width * shaderSelectorMultiplier, rect.y, 15, rect.height);

                            Rect buttonPlusPos = new(shaderSelectorPos.x + shaderSelectorPos.width * buttonPlusPosMultiplier, rect.y, ShaderKeywordButtonLayoutWidth, rect.height);
                            Rect buttonMinusPos = new(buttonPlusPos.x + buttonPlusPos.width, rect.y, ShaderKeywordButtonLayoutWidth, rect.height);
