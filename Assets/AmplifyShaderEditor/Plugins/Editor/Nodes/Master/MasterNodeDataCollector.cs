...
namespace AmplifyShaderEditor
{
    ...
    {
        private List<string> m_names = new();
        private List<Texture> m_values = new();
    ...
    {
        private readonly bool m_showDebugMessages = false;
        ...
        private readonly string m_localVariables;
        ...
        private readonly List<InputCoordsCollector> m_customShadowCoordsList;
        ...
        private readonly TextureChannelUsage[] m_requireTextureProperty = { TextureChannelUsage.Not_Used, TextureChannelUsage.Not_Used, TextureChannelUsage.Not_Used, TextureChannelUsage.Not_Used };
        private readonly WirePortDataType[] m_textureChannelSize =
        {
        ...
        private readonly bool m_dirtyLocalVariables;
        ...
        private readonly Dictionary<MasterNodePortCategory, System.Collections.Generic.HashSet<ParentNode>> m_propagateNodeCache;
        ...
        public void AddToDotsProperties(WirePortDataType dataType, int nodeId, string value, int orderIndex, PrecisionType precision)
        {
            ...
            {
                PropertyDataCollector dataColl = new(nodeId, prop, orderIndex);
        ...
        public void AddToInstancedProperties(WirePortDataType dataType, int nodeId, string value, int orderIndex)
        {
            ...
            {
                PropertyDataCollector dataColl = new(nodeId, value, orderIndex);
                dataColl.DataType = dataType;
        ...
        {
            List<PropertyDataCollector> list = new(m_propertiesDict.Values);
        ...
        public void SoftRegisterUniform(TemplateShaderPropertyData data)
        {
            ...
            {
                PropertyDataCollector newEntry = new(-1, uniformName);
            ...
            {
                PropertyDataCollector newEntry = new(-1, instancedUniform);
        ...
        public void AddToDirectives(string value, int orderIndex = -1, AdditionalLineType type = AdditionalLineType.Custom)
        {
            ...
            {
                PropertyDataCollector data = new(-1, value, orderIndex);
        ...
        public void AddToIncludes(int nodeId, string value)
        {
            ...
            {
                PropertyDataCollector data = new(nodeId, "#include \"" + value + "\"");
        ...
        public void AddToPragmas(int nodeId, string value)
        {
            ...
            if (!m_pragmasDict.ContainsKey(value))
            {
                ...
                PropertyDataCollector dataCollector = new(nodeId, finalValue);
        ...
        public void AddToDefines(int nodeId, string value, bool define = true)
        {
            ...
            if (!m_definesDict.ContainsKey(value))
            {
                ...
                PropertyDataCollector dataCollector = new(nodeId, defineValue);
        ...
        public void UpdateShaderImporter(ref Shader shader)
        {
            ...
            if (m_propertyNodes.Count > 0)
            {
                try
                {
                    ...
                    TextureDefaultsDataColector defaultCol = new();
                    foreach (KeyValuePair<int, PropertyNode> kvp in m_propertyNodes)
                    {
