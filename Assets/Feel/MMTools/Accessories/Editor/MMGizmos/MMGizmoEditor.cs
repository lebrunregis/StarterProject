    ...
    public class MMGizmoEditor : Editor
    {
        ...
        private static void DrawColliderGizmo(MMGizmo mmGizmo)
        {
            ...
            if (mmGizmo._circleCollider2DNotNull)
            {
                if (mmGizmo.ColliderRenderType == MMGizmo.ColliderRenderTypes.Full)
                {
                    Gizmos.DrawSphere(ComputeGizmoPosition(mmGizmo, mmGizmo._circleCollider2D.offset), mmGizmo._circleCollider2D.radius);
                }
                else
                {
                    Gizmos.DrawWireSphere(ComputeGizmoPosition(mmGizmo, mmGizmo._circleCollider2D.offset), mmGizmo._circleCollider2D.radius);
                }
            }
