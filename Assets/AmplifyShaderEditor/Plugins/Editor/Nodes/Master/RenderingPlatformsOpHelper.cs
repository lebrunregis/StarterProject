...
namespace AmplifyShaderEditor
{
    ...
    public class RenderingPlatformOpHelper
    {
        ...
        {
            new(){Label = " Direct3D 11/12", Value = RenderPlatforms.d3d11},
            new(){Label = " OpenGL 3.x/4.x", Value = RenderPlatforms.glcore},
            new(){Label = " OpenGL ES 2.0", Value = RenderPlatforms.gles},
            new(){Label = " OpenGL ES 3.x", Value = RenderPlatforms.gles3},
            new(){Label = " Metal iOS/macOS", Value = RenderPlatforms.metal},
            new(){Label = " Vulkan", Value = RenderPlatforms.vulkan},
            new(){Label = " Xbox One", Value = RenderPlatforms.xboxone},
            new(){Label = " Xbox Series X", Value = RenderPlatforms.xboxseries},
            new(){Label = " PlayStation", Value = RenderPlatforms.playstation},
            new(){Label = " PlayStation 4", Value = RenderPlatforms.ps4},
            new(){Label = " PlayStation 5", Value = RenderPlatforms.ps5},
            new(){Label = " Nintendo Switch", Value = RenderPlatforms.@switch},
        };

        // Values from this dictionary must be the indices corresponding from the list above
        public static readonly Dictionary<RenderPlatforms, int> PlatformToIndex = new()
        {
        ...
        public static readonly List<RenderPlatforms> LegacyIndexToPlatform = new()
        {
            ...
            if (checkedPlatforms > 0 && checkedPlatforms < m_renderingPlatformValues.Length)
            {
                ...
                if (checkedPlatforms < uncheckedPlatforms)
                {
                    ...
                    for (int i = 0; i < m_renderingPlatformValues.Length; i++)
                    {
                        if (m_renderingPlatformValues[i])
                        {
                            result += RenderingPlatformsInfo[i].Value + " ";
                        }
                    }
                }
                else
