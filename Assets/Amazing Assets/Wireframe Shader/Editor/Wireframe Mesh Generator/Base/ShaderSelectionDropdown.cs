...
namespace AmazingAssets.WireframeShader.Editor.WireframeMeshGenerator
{
    internal class ShaderSelectionDropdown : AdvancedDropdown
    {
        ...
        {
            public string shaderFullName;

            public ShaderDropdownItem(string prefix, string fullName, string shaderName)
                : base("  " + shaderName)
            {

                if (fullName == m_SelectedShaderFullName)
                    icon = EditorResources.IconSelected;

                shaderFullName = fullName;
                base.id = (prefix + fullName + shaderName).GetHashCode();
            }
        }


        private readonly Action<object> m_OnSelectedShaderPopup;
        private static string m_SelectedShaderFullName;

        public ShaderSelectionDropdown(Action<object> onSelectedShaderPopup, string selectedShaderFullName)
            : base(new AdvancedDropdownState())
        {
            base.minimumSize = new Vector2(270f, 308f);
            m_OnSelectedShaderPopup = onSelectedShaderPopup;
            m_SelectedShaderFullName = selectedShaderFullName;
        }

        protected override AdvancedDropdownItem BuildRoot()
        {
            AdvancedDropdownItem root = new("Shaders");
        ...
        {
            m_OnSelectedShaderPopup(((ShaderDropdownItem)item).shaderFullName);
        }
        ...
        private void AddShaderToMenu(string prefix, AdvancedDropdownItem parent, string fullShaderName, string shaderName)
        {
            ...
            ShaderDropdownItem shaderDropdownItem = new(prefix, fullShaderName, shaderName);
        ...
        private AdvancedDropdownItem FindOrCreateChild(AdvancedDropdownItem parent, string path)
        {
            ...
            AdvancedDropdownItem advancedDropdownItem = new(text);
