...
namespace AmplifyShaderEditor
{
    ...
    {
        private readonly string NativeFoldoutStr = "Native";

        [SerializeField]
        private List<AdditionalDirectiveContainer> m_additionalDirectives = new();

        [SerializeField]
        private List<AdditionalDirectiveContainer> m_shaderFunctionDirectives = new();

        [SerializeField]
        private List<string> m_nativeDirectives = new();
        ...
        [SerializeField]
        private List<AdditionalDirectiveContainerSaveItem> m_directivesSaveItems = new();
        ...
        private void DrawNativeItemsRect()
        {
            int count = m_nativeDirectives.Count;
        ...
        private void DrawConditionals(Rect rect, AdditionalDirectiveContainer directive)
        {
            var tog = rect;
            tog.width = 15;
            tog.x += 5;
                ...
                m_reordableList = new ReorderableList(m_additionalDirectives, typeof(AdditionalDirectiveContainer), true, false, false, false)
                {
                    ...
                    drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
                    {
                        ...
                        if (m_additionalDirectives[index] != null)
                        {
                            ...
                            Rect popupPos = new(rect.x, rect.y, popUpWidth, EditorGUIUtility.singleLineHeight);
                            ...
                            Rect buttonPlusPos = new(rect.x + rect.width - 2 * Constants.PlusMinusButtonLayoutWidth - conditionalsOffset + 1, rect.y - 2, Constants.PlusMinusButtonLayoutWidth, Constants.PlusMinusButtonLayoutWidth);
                            Rect buttonMinusPos = new(rect.x + rect.width - Constants.PlusMinusButtonLayoutWidth - conditionalsOffset + 1, rect.y - 2, Constants.PlusMinusButtonLayoutWidth, Constants.PlusMinusButtonLayoutWidth);
                            float labelWidthBuffer = EditorGUIUtility.labelWidth;
                            Rect labelPos = new(rect.x + popupPos.width - labelWidthStyleAdjust, rect.y, labelWidthStyleAdjust + rect.width - popupPos.width - buttonPlusPos.width - buttonMinusPos.width + widthAdjust, EditorGUIUtility.singleLineHeight);
        ...
        private void DrawReordableList()
        {
            if (m_reordableList != null)
            {
        ...
        private bool TestConditionals(TemplatePass pass, AdditionalDirectiveContainer directive)
        {
        ...
        private void AddToDataCollector(ref MasterNodeDataCollector dataCollector, TemplatePass pass, TemplateIncludePragmaContainter nativesContainer, bool fromSF)
        {
        ...
        private void AddToDataCollector(ref MasterNodeDataCollector dataCollector, TemplatePass pass, bool fromSF)
        {
            List<AdditionalDirectiveContainer> list = fromSF ? m_shaderFunctionDirectives : m_additionalDirectives;
            int orderIdx = 1;
            int count = list.Count;
        ...
        private void UpdateNativeIndex()
        {
            m_nativeDirectivesIndex = -1;
