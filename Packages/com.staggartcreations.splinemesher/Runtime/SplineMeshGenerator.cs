...
namespace sc.modeling.splines.runtime
{
    public static class SplineMeshGenerator
    {
        ...
        private static readonly List<List<int>> triangles = new();
        private static readonly List<Color> colors = new();

        private static Vector3[] sourceVertices;
        private static readonly List<int[]> sourceTriangles = new();
        private static Vector3[] sourceNormals;
        private static readonly List<Vector4> sourceUv0 = new();
        ...
        private static readonly List<CombineInstance> combineInstances = new();
        ...
        public static readonly Interpolators.LerpFloat FloatInterpolator = new();
            ...
            )
        {
            ...
            float2 trimming = new(settings.distribution.trimStart, settings.distribution.trimEnd);
            for (int splineIndex = 0; splineIndex < splineCount; splineIndex++)
            {
                ...
                float2 trimRange = new((trimming.x / splineLength), 1f - (trimming.y / splineLength));
                ...
                float3 splineScale = new(1f);

                //int splineSamples = 0;

                for (int i = 0; i < segments; i++)
                {
                    float segmentOffset = (i * (segmentLength));
                    float prevZ = -1;
                    ...
                    for (int v = 0; v < sourceVertexCount; v++)
                    {
                        ...
                        if (hasUV)
                        {
                            ...
                            if (settings.mesh.storeGradientsInUV)
                            {
                                ...
                                uv.z = t;
                                //Normalized Height
                                uv.w = math.abs(vertexPositionLocal.y / (meshHeight * splineScale.y));
                            }
                        ...
                        if (hasTangents)
                        {
                            float4 sourceTangent = new(sourceTangents[v]);
        ...
        public static Mesh CreateBoundsMesh(Mesh sourceMesh, int subdivisions = 0, bool caps = false)
        {
            ...
            List<Vector3> mVertices = new();
            List<int> mTriangles = new();
