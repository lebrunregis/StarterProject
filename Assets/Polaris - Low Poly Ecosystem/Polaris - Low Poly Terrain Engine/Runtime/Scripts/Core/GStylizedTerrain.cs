...
namespace Pinwheel.Griffin
{
    ...
    public class GStylizedTerrain : MonoBehaviour
    {
        ...
        private RenderTexture heightMapPolygonal;
        private RenderTexture normalMapPolygonal;
        ...
        {
            if (heightMapPolygonal != null)
            {
                heightMapPolygonal.Release();
                GUtilities.DestroyObject(heightMapPolygonal);
                heightMapPolygonal = null;
            }

            if (normalMapPolygonal != null)
            {
                normalMapPolygonal.Release();
                GUtilities.DestroyObject(normalMapPolygonal);
                normalMapPolygonal = null;
            }
        ...
        public RenderTexture GetPolygonalHeightMap(int resolution)
        {
            if (heightMapPolygonal == null)
            {
                heightMapPolygonal = new RenderTexture(resolution, resolution, 0, GGeometry.HeightMapRTFormat,
                    RenderTextureReadWrite.Linear);
                RenderHeightMap(heightMapPolygonal);
            }

            if (heightMapPolygonal.width != resolution ||
                heightMapPolygonal.height != resolution ||
                heightMapPolygonal.format != GGeometry.HeightMapRTFormat)
            {
                heightMapPolygonal.Release();
                GUtilities.DestroyObject(heightMapPolygonal);
                heightMapPolygonal = new RenderTexture(resolution, resolution, 0, GGeometry.HeightMapRTFormat,
            ...
            if (!heightMapPolygonal.IsCreated())
            {
                RenderHeightMap(heightMapPolygonal);
            }

            return heightMapPolygonal;
        }
        ...
        private void RenderHeightMap(RenderTexture rt, GTerrainChunk chunk)
        {
            ...
            mat.SetVector("_TerrainSize", terrainSize);
            mat.SetTexture("_HeightMap", TerrainData.Geometry.HeightMap);
        ...
        public RenderTexture GetSharpNormalMap(int resolution)
        {
            if (normalMapPolygonal == null)
            {
                normalMapPolygonal = new RenderTexture(resolution, resolution, 0, RenderTextureFormat.ARGBFloat,
                    RenderTextureReadWrite.Linear);
                RenderSharpNormalMap(normalMapPolygonal);
            }

            if (normalMapPolygonal.width != resolution ||
                normalMapPolygonal.height != resolution)
            {
                normalMapPolygonal.Release();
                GUtilities.DestroyObject(normalMapPolygonal);
                normalMapPolygonal = new RenderTexture(resolution, resolution, 0, RenderTextureFormat.ARGBFloat,
            ...
            if (!normalMapPolygonal.IsCreated())
            {
                RenderSharpNormalMap(normalMapPolygonal);
            }

            return normalMapPolygonal;
        }
