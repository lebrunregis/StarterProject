...
internal class ASEFakeInspector : ShaderGUI
{
    ...
    private readonly GUIContent m_button = new("");
    ...
    public static Color LinkColorInPro = new(0.3f, 0.5f, 1);
    public static Color LinkColorInPersonal = new(0.1f, 0.3f, 0.8f);
    private Texture2D m_fetchedImage = null;
    private Texture2D m_defaultImage;

    private readonly string m_jsonURL = "https://amplify.pt/Banner/FakeInteriorInfo.json";
    private BannerInfo m_info = new(
        "https://amplify.pt/Banner/Banner_FakeInterior.jpg",
    ...
    private IEnumerator StartRequest(string url)
    {
        m_webRequest = UnityWebRequest.Get(url);
        yield return m_webRequest.SendWebRequest();
    }

    private IEnumerator StartImageRequest(string url)
    {
    ...
    public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
    {
        if (!m_initialized)
        {
            ...
            m_imageRatio = m_defaultImage.height / (float)m_defaultImage.width;
        ...
        if (m_fetchedImage != null)
        {
            ...
            m_imageRatio = m_fetchedImage.height / (float)m_fetchedImage.width;
...
#endif
            if ((propertyFlags & ((int)ShaderPropertyFlags.HideInInspector | (int)ShaderPropertyFlags.PerRendererData)) == (int)ShaderPropertyFlags.None)
            {
                if (propertyFlags == (int)ShaderPropertyFlags.NoScaleOffset)
