...
namespace AmplifyShaderEditor
{
    ...
    public sealed class BreakToComponentsNode : ParentNode
    {
        ...
        private void UpdateOutputs(WirePortDataType newType)
        {
                ...
                case WirePortDataType.FLOAT2x2:
                    {
                        for (int i = 0; i < 4; i++)
                        {
                            m_outputPorts[i].ChangeProperties("[" + i / 2 + "][" + i % 2 + "]", WirePortDataType.FLOAT, false);
                            m_outputPorts[i].Visible = true;
                        }
                        for (int i = 4; i < m_outputPorts.Count; i++)
                        {
                ...
                case WirePortDataType.FLOAT3x3:
                    {
                        for (int i = 0; i < 9; i++)
                        {
                            m_outputPorts[i].ChangeProperties("[" + i / 3 + "][" + i % 3 + "]", WirePortDataType.FLOAT, false);
                            m_outputPorts[i].Visible = true;
                        }
                ...
                case WirePortDataType.FLOAT4x4:
                    {
                        for (int i = 0; i < 16; i++)
                        {
                            m_outputPorts[i].ChangeProperties("[" + i / 4 + "][" + i % 4 + "]", WirePortDataType.FLOAT, false);
                            m_outputPorts[i].Visible = true;
                        }
                    }
                    break;
        ...
        private string ReturnByType(string value, int outputId)
        {
            switch (m_inputPorts[0].DataType)
            {
                ...
                case WirePortDataType.FLOAT2x2:
                    {
                        return value + "[ " + outputId / 2 + " ][ " + (outputId % 2) + " ]";
                    }
                case WirePortDataType.FLOAT3x3:
                    {
                        return value + "[ " + outputId / 3 + " ][ " + (outputId % 3) + " ]";
                    }
                case WirePortDataType.FLOAT4x4:
                    {
                        return value + "[ " + outputId / 4 + " ][ " + (outputId % 4) + " ]";
                    }
            }
            return value;
        }
