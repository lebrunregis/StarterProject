...
namespace AmplifyShaderEditor
{
    ...
    public sealed class CustomExpressionNode : ParentNode
    {
        ...
        private readonly Dictionary<WirePortDataType, int> WireToIdx = new()
        {
        ...
        [SerializeField]
        private List<CustomExpressionInputItem> m_items = new();
        ...
        [SerializeField]
        private List<CustomExpressionDependency> m_dependencies = new();

        private const float ButtonLayoutWidth = 15;

        private bool m_repopulateNameDictionary = true;
        private readonly Dictionary<string, int> m_usedNames = new();
        ...
        private readonly double m_doubleClickTime = 0.3;
        ...
        private void CheckPortConnection(int portId)
        {
        ...
        private string WrapCodeInFunction(bool isTemplate, string functionName, bool expressionMode)
        {
        ...
        private string GetFileAssetRelativePath()
        {
            ...
            if (currentShader != null)
            {
                ...
                else if (fileAssePath.StartsWith("Assets"))
                {
                    ...
                    Uri path0URI = new(path0, UriKind.Absolute);
                    Uri path1URI = new(path1, UriKind.Absolute);
        ...
        private void DrawItemsAddRemoveInputs()
        {
        ...
        private void DrawDependenciesAddRemoveInputs()
        {
            // Add new port
            if (GUILayoutButton(string.Empty, UIUtils.PlusStyle, GUILayout.Width(ButtonLayoutWidth)))
            {
                ...
                m_dependenciesReordableList = new ReorderableList(m_dependencies, typeof(CustomExpressionDependency), true, false, false, false)
                {
                    ...
                    drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
                    {
                        if (m_dependencies[index] != null)
                        {
                            ...
                            Rect popupPos = new(rect.x, rect.y, rect.width - 2 * Constants.PlusMinusButtonLayoutWidth, EditorGUIUtility.singleLineHeight);
                            Rect buttonPlusPos = new(rect.x + rect.width - 2 * Constants.PlusMinusButtonLayoutWidth, rect.y - 2, Constants.PlusMinusButtonLayoutWidth, Constants.PlusMinusButtonLayoutWidth);
                            Rect buttonMinusPos = new(rect.x + rect.width - Constants.PlusMinusButtonLayoutWidth, rect.y - 2, Constants.PlusMinusButtonLayoutWidth, Constants.PlusMinusButtonLayoutWidth);
                            EditorGUI.BeginChangeCheck();
        ...
        private void DrawReordableInputs()
        {
        ...
        private void RecalculateInOutOutputPorts()
        {
        ...
        private void AddPortAt(int idx)
        {
        ...
        private void RemovePortAt(int idx)
        {
            if (m_inputPorts.Count > m_firstAvailablePort)
            {
        ...
        public string DealWithFileMode(int outputId, ref MasterNodeDataCollector dataCollector, bool ignoreLocalvar)
        {
            ...
            Dictionary<int, CustomExpressionNode> examinedNodes = new();
        ...
        private int CreateOutputId(int inputId)
        {
            return (inputId + 1);
        }

        private int CreateInputId(int outputId)
        {
            return outputId - 1;
        }
