...
namespace AmplifyImpostors
{
    ...
    public class AmplifyTextureImporter : AssetPostprocessor
    {
        ...
        public static Dictionary<string, DataHolder> ImportData = new();
    ...
    public class AmplifyImpostor : MonoBehaviour
    {
        ...
        [NonSerialized]
        private Bounds m_originalBound = new();
        ...
        [NonSerialized]
        private readonly string[] m_standardFileNamesOld = { string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty };

        [NonSerialized]
        private readonly string[] m_standardFileNames = { string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty };
...
#if UNITY_EDITOR
        private static readonly List<KeyValuePair<string, int>> m_hdrpStencilCheck = new()
        {
        ...
        public void SaveTexture(ref RenderTexture tex, string path, ImageFormat imageFormat, int resizeScale, TextureChannels channels, int width = 0, int height = 0)
        {
            ...
            if (imageFormat == ImageFormat.EXR)
                outfile = new Texture2D(width / resizeScale, height / resizeScale, TextureFormat.RGBAFloat, false);
            else
                outfile = new Texture2D(width / resizeScale, height / resizeScale, channels == TextureChannels.RGB ? TextureFormat.RGB24 : TextureFormat.RGBA32, true);

            outfile.name = Path.GetFileNameWithoutExtension(path);
            RenderTexture temp = RenderTexture.active;
            RenderTexture.active = tex;
            outfile.ReadPixels(new Rect(0, 0, width / resizeScale, height / resizeScale), 0, 0);
            ...
            else
            {
                ...
                FileStream FSFile = new(path, FileMode.Create, FileAccess.Write, FileShare.None, BlockSize, false);
                BufIndex = 0;
                do
                {
        ...
        public void CalculatePixelBounds(int targetAmount)
        {
            ...
            Shader packerShader = AssetDatabase.LoadAssetAtPath<Shader>(AssetDatabase.GUIDToAssetPath(PackerGUID));
            Material packerMat = new(packerShader);
            ...
            Texture2D tempTex = new(combinedAlphaTexture.width, combinedAlphaTexture.height, TextureFormat.RGBA32, false);
            ...
            Rect testRect = new(0, 0, tempTex.width, tempTex.height);
            Vector2[][] paths;
            ...
            for (int i = 0; i < paths.Length; i++)
            {
                ...
                {
                    minMaxPoints[index] = paths[i][j] + (new Vector2(tempTex.width * 0.5f, tempTex.height * 0.5f));
        ...
        public void RenderAllDeferredGroups(AmplifyImpostorAsset data = null)
        {
            ...
            List<TextureOutput> outputList = new();
            ...
            Material gbufferToOutputMat = new(gbufferToOutputShader);
            ...
            Shader dilateShader = AssetDatabase.LoadAssetAtPath<Shader>(AssetDatabase.GUIDToAssetPath(DilateGUID));
            Material dilateMat = new(dilateShader);
            ...
            //RenderCombinedAlpha();
            Vector4 offsetCalc = /*transform.worldToLocalMatrix **/ new(m_originalBound.center.x, m_originalBound.center.y, m_originalBound.center.z, 1);
            Vector4 offset = new(offsetCalc.x, offsetCalc.y, offsetCalc.z, -m_pixelOffset.y / m_xyFitSize/*(-pixelOffset.y / m_data.VerticalFrames) * ( m_trueFitsize / m_data.VerticalFrames )*/ );
            Vector4 sizeOffset = new(m_xyFitSize, m_depthFitSize, (m_pixelOffset.x / m_xyFitSize) / m_data.HorizontalFrames, (m_pixelOffset.y / m_xyFitSize) / m_data.VerticalFrames);
            ...
            if (isPrefab)
            {
                ...
                {
                    GameObject mainGO = new("Impostor", new Type[] { typeof(MeshFilter), typeof(MeshRenderer) });
        ...
        private void RenderImpostor(int targetAmount, RenderImpostorMode mode, bool useMinResolution = false, Shader customShader = null)
        {
            ...
            Dictionary<Material, Material> bakeMats = new();
            ...
            List<MeshFilter> validMeshes = new();
            for (int i = 0; i < Renderers.Length; i++)
            {
        ...
        private static Matrix4x4 GetCameraRotationMatrix(ImpostorType impostorType, int hframes, int vframes, int x, int y)
        {
            ...
            {
                Vector3 forw = OctahedronToVector((x / ((float)hframes - 1)) * 2f - 1f, (y / ((float)vframes - 1)) * 2f - 1f);
            ...
            {
                Vector3 forw = HemiOctahedronToVector((x / ((float)hframes - 1)) * 2f - 1f, (y / ((float)vframes - 1)) * 2f - 1f);
                Quaternion octa = Quaternion.LookRotation(new Vector3(forw.x * -1, forw.z * -1, forw.y * -1), Vector3.up);
                camMatrixRot = Matrix4x4.Rotate(octa).inverse;
            }
            return camMatrixRot;
        ...
        {
            Vector3 N = new(oct.x, oct.y, 1.0f - Mathf.Abs(oct.x) - Mathf.Abs(oct.y));
        ...
        {
            Vector3 N = new(x, y, 1.0f - Mathf.Abs(x) - Mathf.Abs(y));
            float t = Mathf.Clamp01(-N.z);
            N.Set(N.x + (N.x >= 0.0f ? -t : t), N.y + (N.y >= 0.0f ? -t : t), N.z);
            N = Vector3.Normalize(N);
            return N;
        ...
        public void GenerateAutomaticMesh(AmplifyImpostorAsset data)
        {
            ...
            Rect testRect = new(0, 0, m_alphaTex.width, m_alphaTex.height);
            Vector2[][] paths;
            ...
            for (int i = 0; i < paths.Length; i++)
            {
                for (int j = 0; j < paths[i].Length; j++)
                {
                    data.ShapePoints[index] = paths[i][j] + (new Vector2(m_alphaTex.width * 0.5f, m_alphaTex.height * 0.5f));
