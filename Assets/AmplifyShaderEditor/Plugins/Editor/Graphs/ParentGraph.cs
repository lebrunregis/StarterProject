...
namespace AmplifyShaderEditor
{
    ...
    public class ParentGraph : ScriptableObject, ISerializationCallbackReceiver
    {
        ...
        [SerializeField]
        private List<ParentNode> m_nodes = new();

        // Sampler Nodes registry
        [SerializeField]
        private UsageListSamplerNodes m_samplerNodes = new();

        [SerializeField]
        private UsageListFloatIntNodes m_floatNodes = new();

        [SerializeField]
        private UsageListTexturePropertyNodes m_texturePropertyNodes = new();

        [SerializeField]
        private UsageListTextureArrayNodes m_textureArrayNodes = new();

        [SerializeField]
        private UsageListPropertyNodes m_propertyNodes = new();

        [SerializeField]
        private UsageListPropertyNodes m_rawPropertyNodes = new();

        [SerializeField]
        private UsageListScreenColorNodes m_screenColorNodes = new();

        [SerializeField]
        private UsageListRegisterLocalVarNodes m_localVarNodes = new();

        [SerializeField]
        private UsageListGlobalArrayNodes m_globalArrayNodes = new();

        [SerializeField]
        private UsageListFunctionInputNodes m_functionInputNodes = new();

        [SerializeField]
        private UsageListFunctionNodes m_functionNodes = new();

        [SerializeField]
        private UsageListFunctionOutputNodes m_functionOutputNodes = new();

        [SerializeField]
        private UsageListFunctionSwitchNodes m_functionSwitchNodes = new();

        [SerializeField]
        private UsageListFunctionSwitchCopyNodes m_functionSwitchCopyNodes = new();

        [SerializeField]
        private UsageListTemplateMultiPassMasterNodes m_multiPassMasterNodes = new();

        [SerializeField]
        private List<UsageListTemplateMultiPassMasterNodes> m_lodMultiPassMasterNodes;

        [SerializeField]
        private UsageListCustomExpressionsOnFunctionMode m_customExpressionsOnFunctionMode = new();

        [SerializeField]
        private UsageListStaticSwitchNodes m_staticSwitchNodes = new();

        [SerializeField]
        private UsageListToggleSwitchNodes m_toggleSwitchNodes = new();
        ...
        private List<ParentNode> m_nodePreviewList = new();

        private Dictionary<int, ParentNode> m_nodesDict = new();

        [NonSerialized]
        private List<ParentNode> m_selectedNodes = new();

        [NonSerialized]
        private List<ParentNode> m_markedForDeletion = new();

        [SerializeField]
        private List<WireReference> m_highlightedWires = new();
        private System.Type m_masterNodeDefaultType;

        [SerializeField]
        private List<PropertyNode> m_internalTemplateNodesList = new();
        private Dictionary<int, PropertyNode> m_internalTemplateNodesDict = new();
        ...
        private void UpdateIdFromNode(ParentNode node)
        {
        ...
        {
            List<ParentNode> unusedNodes = new();
            for (int i = 0; i < m_nodes.Count; i++)
            {
        ...
        private void PropagateHighlightDeselection(ParentNode node, int portId = -1)
        {
            if (portId > -1)
            {
        ...
        private void OnNodeChangeSizeEvent(ParentNode node)
        {
        ...
        public void DrawWires(Texture2D wireTex, DrawInfo drawInfo, bool contextPaletteActive, Vector3 contextPalettePos)
        {
                        ...
                        for (int wireIdx = 0; wireIdx < inputPort.ExternalReferences.Count; wireIdx++)
                        {
                            ...
                            if (reference.NodeId != -1 && reference.PortId != -1)
                            {
                                ...
                                if (outputNode != null)
                                {
                                    ...
                                    Rect portsBoundingBox = new(x, y, width, height);
            ...
            if (m_parentWindow.WireReferenceUtils.ValidReferences())
            {
                if (m_parentWindow.WireReferenceUtils.InputPortReference.IsValid)
                {
                    ...
                    Vector3 startPos = new(inputPort.Position.x, inputPort.Position.y);
        ...
        {
            List<WireBezierReference> list = new();
            for (int i = 0; i < m_wireBezierCount; i++)
            {
                if (m_bezierReferences[i].Contains(position))
        ...
        private void AddToSelectedNodes(ParentNode node)
        {
            node.Selected = true;
        ...
        private void RemoveFromSelectedNodes(ParentNode node)
        {
            node.Selected = false;
            m_selectedNodes.Remove(node);
            node.OnNodeStoppedMovingEvent -= OnNodeFinishMoving;
        ...
        public void OrderNodesByGraphDepth()
        {
            ...
            else
            {
                ...
                List<OutputNode> allOutputs = new();
        ...
        public void CrossCheckTemplateNodes(TemplateDataParent templateData, List<TemplateMultiPassMasterNode> mpNodesList, int lodId)
        {
            ...
            Dictionary<string, TemplateReplaceHelper> nodesDict = new();
        ...
        private void RefreshLinkedMasterNodesInternal(UsageListTemplateMultiPassMasterNodes masterNodes, bool optionsUpdate)
        {
            ...
            if (mpCount > 1)
            {
                Dictionary<string, List<InputPort>> registeredLinks = new();
                for (int i = 0; i < mpCount; i++)
                {
        ...
        private void CheckLinkedPorts(ref Dictionary<string, List<InputPort>> registeredLinks, TemplateMultiPassMasterNode masterNode)
        {
