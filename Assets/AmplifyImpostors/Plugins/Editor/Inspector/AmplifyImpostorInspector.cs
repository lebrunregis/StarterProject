...
namespace AmplifyImpostors
{
    ...
    {
        private bool m_billboardMesh = false;
        private bool m_presetOptions = false;
#if AI_DEBUG_MODE
		bool m_debugOptions = false;
#endif
        private GUIStyle m_foldout;

        private AmplifyImpostor m_instance;
        ...
        private readonly GUIContent[] m_sizesScaleStr = { new("2048"), new("1024"), new("512"), new("256") };

        private readonly int[] m_sizes = { 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192 };
        private readonly GUIContent[] m_sizesStr = { new("32"), new("64"), new("128"), new("256"), new("512"), new("1024"), new("2048"), new("4096"), new("8192") };
        ...
        private static readonly GUIContent AssetFieldStr = new("Impostor Asset", "Asset that will hold most of the impostor data");
        private static readonly GUIContent LODGroupStr = new("LOD Group", "If it exists this allows to automatically setup the impostor in the given LOD Group");
        private static readonly GUIContent RenderersStr = new("References", "References to the renderers that will be used to bake the impostor");
        private static readonly GUIContent BakeTypeStr = new("Bake Type", "Technique used for both baking and rendering the impostor");
        private static readonly GUIContent TextureSizeStr = new("Texture Size", "The texture size in pixels for the final baked images. Higher resolution images provides better results at closer ranges, but are heavier in both storage and runtime.");
        private static readonly GUIContent AxisFramesStr = new("Axis Frames", "The amount frames per axis");
        private static readonly GUIContent PixelPaddingStr = new("Pixel Padding", "Padding size in pixels. Padding expands the edge pixels of the individual shots to avoid rendering artifacts caused by mipmapping.");
        private static readonly GUIContent BakingPresetStr = new("Bake Preset", "Preset object that contains the baking configuration. When empty it will use the standard preset.");
        private static readonly GUIContent LODModeStr = new("LOD Insert Mode", "A rule of how the impostor will be automatically included in the LOD Group");
        private static readonly GUIContent LODTargetIndexStr = new("LOD Target Index", "Target index for the current insert mode");
        private static readonly GUIContent MaxVerticesStr = new("Max Vertices", "Maximum number of vertices that ensures the final created amount does not exceed it");
        private static readonly GUIContent OutlineToleranceStr = new("Outline Tolerance", "Allows the final shape to more tightly fit the object by increasing its number or vertices");
        private static readonly GUIContent NormalScaleStr = new("Normal Scale", "Scales the vertices out according to the shape normals");

        //private Color m_overColor = new Color( 0.65f, 0.85f, 1f );
        private Color m_overColor = new(0.7f, 0.7f, 0.7f);
        private Color m_flashColor = new(0.35f, 0.55f, 1f);
        ...
        private void UpdateDynamicSizes()
        {
            ...
            {
                if (m_currentData.LockedSizes)
                    m_sizesScaleStr[i] = new GUIContent((m_currentData.TexSize.x / AmplifyImpostorBakePresetEditor.TexScaleOpt[i]).ToString());
                else
                    m_sizesScaleStr[i] = new GUIContent(m_currentData.TexSize.x / AmplifyImpostorBakePresetEditor.TexScaleOpt[i] + "x" + m_currentData.TexSize.y / AmplifyImpostorBakePresetEditor.TexScaleOpt[i]);
            }
        }
        ...
        private void FetchShaderTag()
        {
            ...
            {
                Material m = new(m_currentData.Preset.RuntimeShader);
        ...
        private void DelayedBake()
        {
            ...
            {
                GameObject lodgo = new(m_instance.name + "_LODGroup");
        ...
        {
            Triangulator tr = new(points, invertY);
        ...
        private void DrawBillboardMesh(ref bool triangulateMesh, ref bool autoChangeToManual, int cutPreviewSize = 160)
        {
            ...
            Rect texRect = new(5, 5, cutPreviewSize, cutPreviewSize);
            Rect hotRect = new(0, 0, cutPreviewSize + 10, cutPreviewSize + 10);
            ...
            switch (evt.GetTypeForControl(controlID))
            {
                case EventType.MouseDown:
                    if (hotRect.Contains(evt.mousePosition))
                    {
                        ...
                        {
                            Rect handleRect = new(m_currentData.ShapePoints[i].x * cutPreviewSize, m_currentData.ShapePoints[i].y * cutPreviewSize, 10, 10);
            ...
            if (evt.type == EventType.Repaint)
            {
                ...
                Dictionary<string, bool> drawnList = new();
                for (int i = 0; i < m_currentData.ShapePoints.Length; i++)
                {
                ...
                if (m_previewMesh != null && m_instance.m_cutMode == CutMode.Manual)
                {
                    ...
                    for (int i = 0; i < m_previewMesh.triangles.Length - 1; i += 3)
                    {
                        ...
                        Vector3 a = new(m_currentData.ShapePoints[vert].x * cutPreviewSize + 5, m_currentData.ShapePoints[vert].y * cutPreviewSize + 5, 0);
                        Vector3 b = new(m_currentData.ShapePoints[vert2].x * cutPreviewSize + 5, m_currentData.ShapePoints[vert2].y * cutPreviewSize + 5, 0);
                        Vector3 c = new(m_currentData.ShapePoints[vert3].x * cutPreviewSize + 5, m_currentData.ShapePoints[vert3].y * cutPreviewSize + 5, 0);
                ...
                if (m_instance.m_cutMode == CutMode.Manual)
                {
                    for (int i = 0; i < m_currentData.ShapePoints.Length; i++)
                    {
                        Rect handleRect = new(m_currentData.ShapePoints[i].x * cutPreviewSize + 1, m_currentData.ShapePoints[i].y * cutPreviewSize + 1, 8, 8);
                ...
                else
                {
                    for (int i = 0; i < m_currentData.ShapePoints.Length; i++)
                    {
                        Rect handleRect = new(m_currentData.ShapePoints[i].x * cutPreviewSize + 3, m_currentData.ShapePoints[i].y * cutPreviewSize + 3, 4, 4);
